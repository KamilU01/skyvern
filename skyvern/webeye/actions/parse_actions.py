...
def parse_attached_files(text: str | None) -> dict[str, str]:
    """
    Parse attached files section from navigation_goal using Option A format:
...
def parse_actions(
    ...
    # This compensates for LLM not following instructions to extract file URLs
    goal_text = str(task.navigation_goal or "") + " " + str(task.navigation_payload or "")

    # Try structured format first (Option A: "Attached files: - name.pdf: url")
